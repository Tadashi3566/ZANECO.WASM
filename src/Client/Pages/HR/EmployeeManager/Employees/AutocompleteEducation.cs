...
public class AutocompleteEducation : MudAutocomplete<string>
{
    [Inject]
    ...
    private async Task<IEnumerable<string>> SearchText(string value)
    {
        ...
        {
            PageSize = 10,
            AdvancedSearch = new() { Fields = new[] { "education" }, Keyword = value }
        };
        ...
        return _list
            .Where(x => x.Education != string.Empty)
            .Select(x => x.Education).Distinct();
    }
}